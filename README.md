...
Проводка 12 В, USB-C-адаптер, установка на автомобиль и ресурсная проверка ещё
не выполнены.
...
Корневой `platformio.ini` сейчас собирает `Тест 01`. Когда появится основная
прошивка, она будет размещена отдельно в `src/`, как в `shiftlight`.
