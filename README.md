...
`src/main.cpp` — тест прямого соединения Atom Lite и Mini 3A Relay Unit одним
Grove-кабелем. Управляющая линия прямого Grove-порта Atom Lite — GPIO26. Тест
каждые пять секунд меняет состояние реле: при `Relay ON` контакты `COM` и `NO`
должны быть замкнуты, при `Relay OFF` — `COM` и `NC`. Сообщения также выводятся
в последовательный порт на скорости 115200 бод.
