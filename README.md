...
`src/main.cpp` — единственный стендовый тест компонентов. Atom Lite и Mini 3A
Relay Unit соединяются одним Grove-кабелем; управляющая линия прямого
Grove-порта Atom Lite — GPIO26. Короткое нажатие кнопки Atom меняет состояние
реле; сообщения выводятся в последовательный порт на скорости 115200 бод.
