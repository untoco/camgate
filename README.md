...
`src/main.cpp` — экспериментальный тест прямого соединения Atom Lite и Mini 3A
Relay Unit одним Grove-кабелем. Управляющая линия прямого Grove-порта Atom Lite
— GPIO26. Короткое нажатие кнопки Atom меняет состояние реле; сообщения
...
Встроенная кнопка Atom Lite подключена к GPIO39 — входу без внутренней
подтяжки. Поэтому этот режим только для проверки: следи, чтобы реле не меняло
