...
`src/main.cpp` — диагностический тест для подключённого Atom Lite. Он по очереди
включает каждый возможный GPIO на 10 секунд. В этот интервал при корректной
линии управления контакты `COM` и `NO` должны быть замкнуты; сообщения также
выводятся в последовательный порт на скорости 115200 бод.

Тест проверяет GPIO26 прямого Grove-порта Atom Lite и GPIO19, GPIO22, GPIO23,
GPIO33, которые Atomic ToUnit Base может вывести на Grove через DIP-
